...
export default function ReviewsPage() {
  ...
  async function copyPublicLink() {
    ...
    try {
      ...
      toast.success(french ? "Lien public copié" : "Public review link copied");
    } catch { toast.error(french ? "Impossible de copier le lien" : "Could not copy link"); }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">{french ? "Avis" : "Reviews"}</h2>
          <p className="text-sm text-gray-500">{french ? "Gérez les commentaires publics et protégez la réputation de votre entreprise." : "Manage public feedback and protect your business reputation."}</p>
        </div>
        {reviewUrl && (
          <div className="flex items-center gap-2">
            <button onClick={copyPublicLink} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
              <Copy className="h-4 w-4" /> {french ? "Copier le lien" : "Copy link"}
            </button>
            <a href={reviewUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-3 py-2 text-sm font-semibold text-white hover:bg-violet-700">
              <ExternalLink className="h-4 w-4" /> {french ? "Page publique" : "Public page"}
            </a>
          </div>
        )}
      </div>
      {loadError ? (
        ...
        <EmptyState title={french ? "Aucun avis" : "No reviews yet"} icon={Star} description={french ? "Marquez les rendez-vous comme terminés pour envoyer automatiquement des demandes d’avis sécurisées." : "Mark completed appointments to send signed review requests automatically. The more clients you book, the more reviews roll in."} action={{ label: french ? "Partager votre lien de réservation" : "Share your booking link", href: "/dashboard/booking-page" }} />
      ) : (
        <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
          <div className="space-y-4">
            <Card>
              <CardContent className="py-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">{french ? "Note publique" : "Public rating"}</p>
                ...
                <p className="mt-2 text-sm text-gray-500">{published.length} {french ? "publiés" : "published"} · {hidden} {french ? "masqués" : "hidden"}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-5 space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">{french ? "Répartition des notes" : "Rating breakdown"}</p>
                ...
                <CardContent className="py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-1">{formatDate(r.createdAt, { year: "numeric", month: "short", day: "numeric" })}{r.published ? (french ? " · public" : " · public") : (french ? " · masqué" : " · hidden")}</p>
                    </div>
                    <button onClick={() => toggle(r)} aria-label={r.published ? (french ? "Masquer l’avis" : "Hide review") : (french ? "Publier l’avis" : "Publish review")} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-semibold text-gray-600 hover:border-violet-300 hover:text-violet-700 shrink-0">
                      {r.published ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                      {r.published ? (french ? "Masquer" : "Hide") : (french ? "Publier" : "Publish")}
                    </button>
                  </div>
                  {r.comment ? <p className="text-sm text-gray-600 mt-3 leading-relaxed">{r.comment}</p> : <p className="text-sm text-gray-400 mt-3">{french ? "Aucun commentaire écrit." : "No written comment."}</p>}
                </CardContent>
              </Card>
            ))}
          </div>
