    ...
    <div className="max-w-3xl">
      <div className="mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-900">{french ? "Page de réservation" : "Booking Page"}</h1>
        </div>
        <p className="text-sm text-gray-500 mt-1">{french ? "Votre vitrine publique — partagez ce lien pour permettre aux clients de réserver en tout temps." : "Your public storefront — share the link so clients can book themselves, 24/7."}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">{french ? "Votre lien de réservation" : "Your booking link"}</p>
            <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2.5 border border-gray-200">
              ...
              <button type="button" onClick={copyLink} aria-label={french ? "Copier le lien de réservation" : "Copy booking link"} className="shrink-0 text-gray-500 hover:text-violet-700">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <a href={bookingUrl} target="_blank" rel="noopener noreferrer"
                ...
                <ExternalLink className="w-4 h-4" /> {french ? "Aperçu" : "Preview page"}
              </a>
              ...
              <a href={`mailto:?subject=${encodeURIComponent(french ? "Prendre rendez-vous" : "Book an appointment")}&body=${encodeURIComponent(french ? `Vous pouvez prendre rendez-vous ici : ${bookingUrl}` : `You can book an appointment with me here: ${bookingUrl}`)}`}
                className="inline-flex items-center rounded-lg border border-gray-200 px-3.5 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">{french ? "Courriel" : "Email"}</a>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <Link href="/dashboard/settings?tab=branding" className="flex items-center justify-between rounded-xl border border-gray-100 bg-white p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-800">{french ? "Image de marque" : "Branding"}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{french ? "Couleurs, polices et style de page" : "Colors, fonts, page style"}</p>
                </div>
              </div>
            ...
            <Link href="/dashboard/settings?tab=online" className="flex items-center justify-between rounded-xl border border-gray-100 bg-white p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-800">{french ? "Intégration et options avancées" : "Embed & advanced"}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{french ? "Code d’intégration et lien bio" : "Website embed snippet, bio link"}</p>
                </div>
              </div>
              <ChevronRight className="w-4 h-4 text-gray-400 shrink-0" />
            </Link>
