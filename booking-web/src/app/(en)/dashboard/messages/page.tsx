...
export default function MessagesPage() {
  ...
  async function openThread(t: Thread) {
    ...
    try {
      ...
      loadThreads();
    } catch (e) { setThreadError(e instanceof Error ? e.message : (french ? "Échec du chargement de la conversation" : "Failed to load thread")); }
  ...
  async function send() {
    ...
    try {
      ...
      if (res?.sms?.sent) toast.success(french ? "Envoyé dans l’application et par texto au client" : "Sent and texted to the client");
      else if (res?.sms?.reason === "client_must_text_first") toast.success(french ? "Envoyé dans l’application. Le SMS sera disponible après le premier message du client." : "Sent in-app. SMS is available after the client texts first.");
      else if (res?.sms?.reason === "send_failed") toast.warning(french ? "Envoyé dans l’application, mais le SMS n’a pas pu être livré." : "Sent in-app, but the SMS could not be delivered.");
      else if (res?.sms?.reason === "no_phone") toast.success(french ? "Envoyé dans l’application. Ce client n’a pas de numéro de téléphone pour le SMS." : "Sent in-app. This client has no phone number for SMS.");
      else if (res?.sms?.reason === "plan_not_eligible") toast.success(french ? "Envoyé dans l’application." : "Sent in-app.");
      loadThreads();
  ...
  async function setSelectedArchived(archived: boolean) {
    ...
    try {
      ...
      toast.success(archived ? (french ? "Conversation archivée" : "Conversation archived") : (french ? "Conversation restaurée" : "Conversation restored"));
    } catch { toast.error(french ? "Échec de la mise à jour de la conversation" : "Failed to update conversation"); }
  }
    ...
    <div className="dashboard-dynamic-height max-w-5xl mx-auto flex min-h-[28rem] gap-4">
      ...
      <div className={cn(
        ...
        <div className="px-4 py-3.5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-900">{french ? "Conversations" : "Conversations"}</h3>
          {unread > 0 && (
            <span className="text-xs font-bold text-white bg-violet-600 rounded-full px-2 py-0.5">{unread}</span>
          )}
        </div>
        <div className="p-3 border-b border-gray-100 space-y-2">
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={french ? "Rechercher des messages" : "Search messages"} />
          <div className="flex gap-1">
            {(["all", "unread", "archived"] as const).map((value) => <button key={value} onClick={() => setFilter(value)} className={cn("rounded-full px-2.5 py-1 text-xs font-semibold capitalize", filter === value ? "bg-violet-600 text-white" : "bg-gray-100 text-gray-500")}>{french ? ({ all: "tous", unread: "non lus", archived: "archivés" } as const)[value] : value}</button>)}
          </div>
          <div className="flex gap-1">
            {([
              { val: "ALL", icon: null, label: "All" },
              { val: "IN_APP", icon: <Mail className="w-3 h-3" />, label: french ? "Interne" : "In-app" },
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
          {loadError ? (
            <div className="text-center py-10 px-4">
              ...
              <button onClick={() => { setLoadError(""); loadThreads(); }} className="text-violet-600 hover:underline text-sm">{french ? "Réessayer" : "Retry"}</button>
            </div>
          ) : loading ? <LoadingSpinner className="py-8" /> :
           threads.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center px-4">
              <MessageSquare className="w-8 h-8 text-gray-200 mb-2" aria-hidden="true" />
              <p className="text-sm text-gray-400">{french ? "Aucun message" : "No messages yet"}</p>
              <p className="text-xs text-gray-600 mt-1">{french ? "Les clients peuvent écrire depuis leur portail" : "Clients can message from their portal"}</p>
            </div>
          ) : threads.map((t) => (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {t.unreadCount > 0 && <span className="rounded-full bg-red-600 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white">{t.unreadCount} {french ? "non lus" : "unread"}</span>}
                    </div>
      ...
      <div className={cn(
        ...
        {!selected ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-sm font-medium text-gray-500">{french ? "Sélectionner une conversation" : "Select a conversation"}</p>
            <p className="text-xs text-gray-400 mt-1">{french ? "Choisissez un client à gauche pour voir les messages" : "Pick a client from the left to view messages"}</p>
          </div>
        ) : (
          <>
            ...
            <div className="px-4 sm:px-5 py-3.5 border-b border-gray-100 flex items-center gap-2 sm:gap-3">
              <button onClick={() => setSelected(null)}
                className="md:hidden -ml-1 p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 transition-colors" aria-label={french ? "Retour aux conversations" : "Back to conversations"}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-gray-400 truncate">{selected.client.email || (french ? "Contact téléphonique" : "Phone contact")}</p>
              </div>
              <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-semibold text-gray-400 bg-gray-50 border border-gray-100 rounded-full px-2 py-1 shrink-0">
                <Mail className="w-3 h-3" /> {french ? "Interne" : "In-app"} · <Smartphone className="w-3 h-3" /> SMS
              </span>
              <button onClick={() => setSelectedArchived(!selected.archived)} aria-label={selected.archived ? (french ? "Restaurer la conversation" : "Restore conversation") : (french ? "Archiver la conversation" : "Archive conversation")} className="text-xs font-semibold text-gray-500 hover:text-red-600">
                {selected.archived ? (french ? "Restaurer" : "Restore") : (french ? "Archiver" : "Archive")}
              </button>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-3">
              {threadError && (
                <div className="text-center py-8">
                  ...
                  <button onClick={() => { setThreadError(""); if (selected) openThread(selected); }} className="text-violet-600 hover:underline text-sm">{french ? "Réessayer" : "Retry"}</button>
                </div>
              )}
              {!threadError && msgs.length === 0 && (
                <p className="text-sm text-gray-400 text-center py-8">{french ? "Aucun message" : "No messages yet"}</p>
              )}
              {msgs.map((m) => (
            ...
            <div className="dashboard-safe-bottom px-3 sm:px-4 py-3 border-t border-gray-100 flex items-end gap-2">
              <Input
                placeholder={french ? "Écrire une réponse…" : "Type a reply…"}
                value={reply}
                onChange={(e) => setReply(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
                className="flex-1"
              />
              <Button size="sm" onClick={send} loading={sending} disabled={!reply.trim()} aria-label={french ? "Envoyer le message" : "Send message"}>
