            ...
            <Card key={e.id}>
              <CardContent className="py-4 flex items-center gap-4">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <a href={`mailto:${e.email}?subject=${encodeURIComponent(french ? "Une place de réservation est disponible" : "A booking spot is available")}&body=${encodeURIComponent(french ? `Bonjour ${e.name}, une place pourrait être disponible. Répondez à ce courriel ou réservez en ligne si l’horaire vous convient.` : `Hi ${e.name}, a spot may be available. Reply to this email or book online if the time works for you.`)}`} className="text-gray-400 hover:text-blue-600 p-2" title={french ? "Aviser" : "Notify"} aria-label={french ? "Aviser le client" : "Notify client"}><Send className="w-4 h-4" /></a>
                  <Link href="/dashboard/appointments" className="text-gray-400 hover:text-violet-600 p-2" title={french ? "Réserver" : "Book"} aria-label={french ? "Prendre un rendez-vous" : "Book appointment"}><CalendarPlus className="w-4 h-4" /></Link>
                  <button type="button" onClick={() => setEntryToRemove(e)} className="text-gray-400 hover:text-red-600 p-2" title={french ? "Retirer" : "Remove"} aria-label={french ? "Retirer de la liste d’attente" : "Remove from waitlist"}><Trash2 className="w-4 h-4" /></button>
                </div>
              </CardContent>
            </Card>
          ))}
