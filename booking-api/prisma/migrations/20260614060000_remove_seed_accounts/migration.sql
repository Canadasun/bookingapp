...
-- 1. Seed user accounts (cascade on FK means related sessions/devices are removed too)
DELETE FROM "User"
WHERE email IN (
...
BEGIN
  ...
  WHERE "businessId" = biz_id
    AND email NOT IN ('owner@demo-salon.com', 'stylist@demo-salon.com', 'pmayeni1@icloud.com');
